...
import Data.Sequence as S ( Seq, Seq ( (:<|) ), (><), fromList, splitAt )
...
main = do
        ...
        let (crates:movements:_) = map lines . splitOn "\n\n" $ input
            initialState = map (filter (/= ' ')) . transpose . map (map (!! 1) 
                            . chunksOf 4) $ crates
            initialStateString = I.fromList . map (\x -> (digitToInt . last $ x
                                                  , init x)) $ initialState
            movedPile = foldl newPileFromString initialStateString movements 
            initialStateSeq = I.fromList . map (\x -> (digitToInt . last $ x
                                                , S.fromList . init $ x)) 
                                                $ initialState
