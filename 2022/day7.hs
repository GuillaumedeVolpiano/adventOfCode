...
type Name = String
type Size = Int
data FSItem = File Size Name | Folder Name [FSItem]
    deriving (Show)
data FSPos = FSPos Name [FSItem] [FSItem]
    deriving (Show)
...
populate (x:xs) (Folder foldername items, bs)
    | firstWord == "dir" = populate xs 
                        (Folder foldername (Folder secondWord [] : items), bs)
    | otherwise = populate xs 
...
sizeList folder@(Folder name items) list = folderSize folder : populate items list
        where
