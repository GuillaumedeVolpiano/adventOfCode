...
main = do
  ...
  let year = read $ directory =~ "[0-9]+"
      day = read $ prog =~ "[0-9]+"
      isTest = not . L.null $ args
  input <- retrieveInput year day args
  let initialState = parseInput . lines $ input
      rounds = iterate doRound initialState
  putStrLn "part 1"
  preciseTimeIt 3 . print . countEmpty . last . take 11 $ rounds
  putStrLn "part 2"
  preciseTimeIt 3 . print $
    (length . takeWhile (\(State a _, State b _) -> a /= b) . zip rounds $
     tail rounds) +
    1
