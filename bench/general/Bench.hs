...
tests input =
  [ env (Day4.mkMap . S.fromList . BS.unpack $ input) $ \rs -> bench "Part 1" $ whnfIO $ Day4.countAccessible Day4.removeFree rs
  , env (Day4.mkMap . S.fromList . BS.unpack $ input) $ \rs -> bench "Part 2" $ whnfIO $ Day4.countAccessible Day4.purge rs ]
