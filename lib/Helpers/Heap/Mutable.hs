...
import           Data.Vector.Mutable (IOVector, unsafeNew, unsafeRead,
                                      unsafeWrite)
import           GHC.IORef           (IORef, newIORef, readIORef, writeIORef)

data Heap a = H (a -> a -> Ordering) !(IOVector a) !(IORef Int)

-- | Allocate a unsafeNew heap with given capacity and ordering.
newHeap :: Int -> (a -> a -> Ordering) -> IO (Heap a)
newHeap sz ord = do
  pq <- unsafeNew sz
  s <- newIORef 0
  pure $ H ord pq s
